...
if not exist "%dll_file_backup%" (
	mkdir "..\backup"
	move "..\steam_api.dll" "%dll_file_backup%" > nul
	if exist "%dll_file_backup%" (
		echo Successfully backed up original %appid% steam_api.dll!
	)
)
if not exist "%dll64_file_backup%" (
	mkdir "..\backup"
	move "..\steam_api64.dll" "%dll64_file_backup%" > nul
	if exist "%dll64_file_backup%" (
		echo Successfully backed up original %appid% steam_api64.dll!
	)
)
...
copy "%dll64_file_goldberg%" .. > nul
if exist "..\steam_api64.dll" (
	echo Successfully copied Goldberg steam_api64.dll to dll folder!
)

pause

::create emu config::
echo python %script_file% %USERNAME% %PASSWORD% %appid%
call python %script_file% %USERNAME% %PASSWORD% %appid%
echo Successfully created emu config!
echo.

::move emu config to game directory::
move "%appid%_output\steam_settings" ..
