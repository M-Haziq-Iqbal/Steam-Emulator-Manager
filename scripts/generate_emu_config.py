...
from scripts.stats_schema_achievement_gen import achievements_gen
from scripts.controller_config_generator import parse_controller_vdf
    ...
    for x in steam_id_list:
        ...
        if out is not None:
            if len(out.body.schema) > 0:
                with open(os.path.join(backup_directory, 'UserGameStatsSchema_{}.bin'.format(game_id)), 'wb') as f:
        ...
        if "config" in game_info:
            if "steamcontrollerconfigdetails" in game_info["config"]:
                ...
                for id in controller_details:
                    ...
                    print(f"\t\tCreating {id} {controller_type}")
